...
Class ingresos
{
	...
	$cont=0;
		...
		$sql="SELECT CONCAT(c.tiponac,c.cedularif) AS cedularif  FROM mayor m INNER JOIN citizen c ON m.idrfc=c.rfc WHERE m.tramite='$tramite' LIMIT 1;";
  ...
  public function reportedeldia($comodinbusqueda,$comodinbusqueda2)
	{
      ...
      //SELECT SUM(d.mount) AS monto FROM `cpdv` c LEFT JOIN dtcpdv d ON c.id=d.cpdv_id WHERE c.tributo=1024 AND DATE_FORMAT(c.fecha, '%Y-%m-%d')='2024-02-08' AND d.ptype=0 GROUP BY c.tributo; 
...
FROM (SELECT t.partida,t.detalle,c.tributo,DATE_FORMAT(c.fecha, '%d/%m/%Y') AS fecha,SUM(c.monto) as monto FROM cpdv c LEFT JOIN dtcpdv d ON c.id=d.cpdv_id LEFT JOIN tributes t ON t.idt=c.tributo WHERE DATE_FORMAT(c.fecha, '%Y-%m-%d') BETWEEN '$comodinbusqueda' AND '$comodinbusqueda2' AND d.ptype=0 GROUP BY c.tributo UNION SELECT t.partida,t.detalle,c.tributo,DATE_FORMAT(c.fecha, '%d/%m/%Y') AS fecha,SUM(c.monto) as monto FROM cpdv_bank B, cpdv c LEFT JOIN dtcpdv d ON c.id=d.cpdv_id LEFT JOIN tributes t ON t.idt=c.tributo WHERE c.P='C' AND DATE_FORMAT(c.momemt, '%Y-%m-%d') BETWEEN '$comodinbusqueda' AND '$comodinbusqueda2'  AND (d.ptype=1 OR d.ptype=5) AND B.isused=1 and  B.refencia=d.ref GROUP BY c.tributo) a GROUP BY tributo";
	...
	public function resumendeingresosdetalleDebito($comodinbusqueda,$comodinbusqueda2)//INGRESOS DIARIO DEIALLE
	{
     ...
     /*  $sql="SELECT partida,fecha,movimiento,banco,codbanco,approval,ref,monto
FROM (SELECT t.partida,DATE_FORMAT(c.fecha, '%d/%m/%Y') AS fecha,'DEBITO' AS movimiento,'BNC' AS banco,'497' AS codbanco,d.approval,d.ref,SUM(c.monto) as monto FROM cpdv c LEFT JOIN dtcpdv d ON c.id=d.cpdv_id LEFT JOIN tributes t ON t.idt=c.tributo WHERE DATE_FORMAT(c.fecha, '%Y-%m-%d')='$comodinbusqueda' AND d.ptype=0 GROUP BY c.ctramite,d.approval,d.ref UNION SELECT t.partida,DATE_FORMAT(c.fecha, '%d/%m/%Y') AS fecha,'' AS movimiento,'' AS banco,'' AS codbanco,d.approval,d.ref,c.monto as monto FROM cpdv_bank B, cpdv c LEFT JOIN dtcpdv d ON c.id=d.cpdv_id LEFT JOIN tributes t ON t.idt=c.tributo WHERE c.P='C' AND DATE_FORMAT(c.momemt, '%Y-%m-%d')='$comodinbusqueda'  AND (d.ptype=1 OR d.ptype=5) AND B.isused=1 and  B.refencia=d.ref) a ORDER BY movimiento DESC";*/
