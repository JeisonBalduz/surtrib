...
switch ($_GET["op"]){
	...
	case 'guardaryeditar':
		if (empty($id)){
			$rspta=$definitiva->insertar($rfc2,$montobrutoanual);
	...
	case 'listar':
		...
		$rspta=$definitiva->listar();
 		...
 		while ($reg=$rspta->fetch_object()){
 			$data[]=array(
				"0"=>'<a target="_blank" href="../reportesPDF/definitiva.php?codigo='.$reg->id.'&codigo2='.$reg->rfc.'");" class="btn btn-info">Ver Definitiva</a>',
				"1"=>$reg->rfc,
				"2"=>$reg->rif,
				"3"=>$reg->name,
			);
