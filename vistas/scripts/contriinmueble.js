...
function chequear_ampos(){    ///alert("Hola");
    if($("#CC_Uso").val()=="-1"){
     ...
     if ($("#CC_Tipo").val()=="-1"){
        alert("Seleccione Tipo");$('#CC_Tipo').focus();
         return false;
    }
    else
     if ($("#CC_Ocup").val()=="-1"){  
        alert("Seleccione Ocupacion");$('#CC_Ocup').focus();
         return false;
    }
    else
     if ($("#CC_Estr").val()==""){  
        alert("Seleccione Estructura");$('#CC_Estr').focus();
         return false;
    }
    else
     if ($("#CC_Piso").val()=="-1"){
        alert("Seleccione Tipo de Piso");$('#CC_Piso').focus();
         return false;
    }
    else
     if ($("#CC_Puer").val()==""){
        alert("Seleccione Tipo de Puertas");$('#CC_Puer').focus();
         return false;
    }
    else  
     if ($("#CC_Par_Tipo").val()=="-1"){
        alert("Seleccione Tipo de Paredes");$('#CC_Par_Tipo').focus();
         return false;
    }
    else
     if ($("#CC_Par_Acab").val()=="-1"){
        alert("Seleccione Tipo de Acabado");$('#CC_Par_Acab').focus();
         return false; 
    }
    else
     if ($("#CC_Tech_Estr").val()=="-1"){
        alert("Seleccione Tipo de Estrutura");$('#CC_Tech_Estr').focus();
         return false;
    }
    else 
     if ($("#CC_Tech_Cubi").val()=="-1"){
        alert("Seleccione Tipo de Cubiertas");$('#CC_Tech_Cubi').focus();
         return false;
    }
    else
     if ($("#CC_Elec").val()=="-1"){
        alert("Seleccione Tipo Empotrado electrico");$('#CC_Elec').focus();
         return false;
    }
    else     
     if ($("#CC_Est_Con").val()=="-1"){
        alert("Seleccione Estado de Conservacion");$('#CC_Est_Con').focus();
         return false;
    }
     else  
     if ($("#CC_Clas").val()=="-1"){
        alert("Seleccione La Clasificacion");$('#CC_Clas').focus();
         return false;
    }
    else
     if ($("#CC_NH").val()==""){
        alert("Ingrese Numero de habitaciones");$('#CC_NH').focus();
         return false;
    }
    else
     if ($("#CC_NP").val()==""){
        alert("Ingrese Numero de Plantas");$('#CC_NP').focus();
         return false;
    }
    else  
     if ($("#Anio_Cons").val()=="0"){
        alert("Seleccione Año de la Construccion");$('#Anio_Cons').focus();
         return false;
    }
    else
     if ($("#CC_Dim_Fre").val()==""){
        alert("Ingrese Las Dimenciones del Frente");$('#CC_Dim_Fre').focus();
         return false;
    }
    else
     if ($("#CC_Dim_Fon").val()==""){
        alert("Ingrese Las Dimenciones del Fondo");$('#CC_Dim_Fon').focus();
         return false;
    }
    if ($("#CC_Alic").val()=="-1"){
        alert("Seleccione la Alicuota");$('#CC_Alic').focus();
         return false;
    }
     return true;
}

function getnuevaconstruccion(){
    var query="&r=" + new Date().getTime()+";";

    /* var op = chequear_ampos();
         if(op){*/
            $.post("../ajax/ajaxInmueble.php?op=opcionesinmueble"+query, {
                tipoopcion:"construccionedit",
                Id_Const:""
            }, function (e) {
                bootbox.confirm({   title: 'DATOS DE LA CONSTRUCCIOM',
                                message: ''+e,
                                size: 'extra-large',
                                buttons: {
                                confirm: {
                                ...
                                callback: function (result) {
                                    if (result){
                                        ...
                                        var opt=guardarConstruccion('nuevaconstruccion');
                                       return opt;
                                        //tabla2.ajax.reload(); false para que no cierre
                                       // alert("Hola");
                                    }
