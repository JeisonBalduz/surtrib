...
$tramite = "SELECT u.rfc,u.name,u.rif,c.id,cc.id AS ccid,c.annomm,c.income,c.tax,c.deductible,cc.moment,cc.tramite,cc.rfc,a.detalles,a.codigo_grupo,a.minimo_tributable,a.alicuota FROM companyeib c LEFT JOIN companye cc ON c.idrelcompanye=cc.id LEFT JOIN activitiesenero2024 a ON c.idactivity=a.id LEFT JOIN users u ON cc.rfc=u.usuario WHERE cc.id IN (".$codigo.") ORDER BY c.annomm DESC";
...
$pdf->SetWidths(array(15,96,15,25,25,20));
$pdf->Row(array(utf8_decode('Codigo'),utf8_decode('Descricion Actividad'),utf8_decode('Alicuota'),utf8_decode('Minimo Tributable'),utf8_decode('Ingresos Brutos (Bs)'), utf8_decode('Impuesto Mes')));
...
$pdf->SetAligns(array('J','J','J','J','J','J'));
$pdf->SetWidths(array(15,96,15,25,25,20));
$pdf->Row(array(utf8_decode($row['codigo_grupo']),utf8_decode($row['detalles']),utf8_decode($row['alicuota']),utf8_decode($row['minimo_tributable']),utf8_decode($row['income']), utf8_decode($row['tax'])));
...
$pdf->SetAligns(array('J','J','J'));
$pdf->SetWidths(array(65,66,65));
$pdf->Row(array(utf8_decode('Total de ingresos: '.$total.''),utf8_decode('Total de impuestos: '.$taxt.''),utf8_decode('Total de retenciones: 0.00')));
