...
$tramite = "SELECT u.name,u.rfc,u.rif,m.tramite,c.sector,c.calle,c.edificio,c.numeroedif,m.id FROM mayor_aseo m LEFT JOIN users u ON m.idrfc=u.rfc LEFT JOIN citizen c ON c.rfc=u.rfc WHERE m.tramite IN (".$codigo.") GROUP BY m.tramite limit 1";
$tramite2 = "SELECT m.id,m.idt,t.detalle,m.totliq,m.totpag,m.descuento,m.period FROM `mayor_aseo` m LEFT JOIN tributes_aseo t ON t.idt=m.idt WHERE `tramite` IN (".$codigo.") ";
$tramite3 = "SELECT SUM(`totpag`) AS calculo, SUM(`descuento`) AS caldescuento, SUM(`creditof`) AS calcredito, SUM(`totliq`) AS caltotliq FROM `mayor_aseo` WHERE tramite IN (".$codigo.")";
$link = $conexion;
...
while ($obj = mysqli_fetch_object($result)) {
    ...
    $number = "".$idmayor;
    $length = 11;
    $idmayor=substr(str_repeat('0', $length).$number, - $length);
...
//$this->Image('img/logoa.jpg',180,8,25);
//$this->Image('img/escudo.png',10,9,11);
$this->Image('img/logo.jpg',180,8,25);//,12,9,30);
...
$pdf->Cell(196, 8, utf8_decode('RECIBO DE PAGO NRO '.$idmayor),1,0,'C',1);
